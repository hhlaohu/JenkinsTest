var YDataGrid = function(config){
		...
		var Handler = {
			...
			close : function (callback){
				$.messager.confirm('确认','确定关闭窗口?',function(r){  
				    if (r){  
				     	Win.edit.dialog('close');
				     	//回调函数
						if(jQuery.isFunction(callback)){
		...
		var initForm = function(){
			if(Form.search && Form.search[0]){
				...
				$(document).keyup(function(event){//回车事件，执行查询按钮
					  if(event.keyCode ==13){
					    Form.search.find("#btn-search").trigger("click");
					  }
				}); 
			}
		}
		...
		//初始化方法
		this.init = function(){
//			$(".panel-title panel-with-icon",'').html('');
			initGrid();
			initTbar();
			initForm();
			initWin();
